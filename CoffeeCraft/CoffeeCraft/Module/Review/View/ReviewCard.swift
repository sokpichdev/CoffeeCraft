...
#Preview {
    ScrollView {
        VStack(spacing: 14) {
            // Own review — highlighted variant
            ReviewCard(
                review: .preview(
                    userName: "Sok Pich",
                    rating: 5,
                    body: "Best matcha latte I've had in ages. The oat milk ratio is perfect and it's never too sweet.",
                    helpfulCount: 3,
                    isOwn: true
                ),
                vm: ReviewViewModel(),
            ...
            // Regular review with body
            ReviewCard(
                review: .preview(
                    userName: "Jane Smith",
                    rating: 4,
                    body: "Really good, just a tiny bit too sweet for my taste. Will definitely order again.",
                    helpfulCount: 1
                ),
                vm: ReviewViewModel()
            )
            ...
            // Stars only — no body
            ReviewCard(
                review: .preview(userName: "Alex K.", rating: 3, helpfulCount: 0),
                vm: ReviewViewModel()
            )
...
// MARK: - Review preview helper (only in DEBUG)
...
extension Review {
    static func preview(
        ...
        Review(
            id: UUID().uuidString,
            userId: isOwn ? (UserSession.shared.userId ?? "own") : UUID().uuidString,
            userName: userName,
            ...
            helpfulBy: [],
            createdAt: .init(date: Date().addingTimeInterval(-86400 * 3)),
            updatedAt: nil,
            isHidden: false
        )
    }
